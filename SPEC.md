...
│   │   ├── TodoViewModel.swift
│   │   └── TodoService.swift
│   ├── QuickCapture/              # 빠른 추가 (구 "빠른 캡처", 투두 탭 플로팅 버튼 → 시트)
...
#### 투두 알림
- UNUserNotificationCenter 사용. identifier는 `todo.id`
- 시간 지정된 투두에 한해 알림 설정 가능
- 알림 시간: 정시 / 5분 전 / 10분 전 / 15분 전 / 30분 전 / 1시간 전 / 2시간 전 / 1일 전 / 2일 전 / 1주 전 / 직접 입력
- **직접 입력 (2026-09-23):** 「없음」/「완료」. 숫자 키패드 1~999, 단위는 분·시간·일·주·개월. 프리셋이 아닌 값은 메뉴에 표시(예: 45분 전). 다시 열면 저장된 값에서 시작
- **문구 (2026-09-23):** 제목은 할 일 이름. 본문은 울리는 날(`fireDate`)과 예정일의 날짜 차이로 「오늘 / 내일 / 모레 + 시각」. 현재 시각과 비교하지 않음. 그 밖은 날짜·요일·시각. 메모는 넣지 않음
- `alarmOffset`이 없으면 기존 알림을 취소. `scheduledTime`이 없으면 기존 알림은 유지
- **앱 사용 중:** `AppNotificationDelegate`가 할 일 알림도 배너·알림 목록·소리로 표시. 탭 동작은 그대로

#### 빠른 추가 (구 "빠른 캡처")
> 명칭 변경 이유: "캡처"가 스크린샷 캡처로 오해될 수 있어 사용설명서 작성 중 "빠른 추가"로 변경 확정
- **투두 탭** 우하단 플로팅 ＋ 버튼 (모든 화면 아님 — 이전 스펙 오류 수정)
...
- 진입: ① 투두탭 ⋯ 「카테고리 설정」시트 (왼쪽 「편집」·오른쪽 「+」) ② 설정 → 플래너 → 「카테고리 관리」push (왼쪽 `<`·오른쪽 「편집」+「+」). 목록·추가·편집 동작은 동일
...
**삭제 vs 숨김 (구 "보관", 노션 연동 시 확인 팝업):**

| 동작 | 앱 | 노션 |
|---|---|---|
| **삭제** | `CategoryItem` 영구 삭제, 연결 투두 `categoryId` 해제 | 연결된 카테고리면 select/status **옵션도 삭제** (`remove-select-option`) |
| **숨김** | `status = archived`, 활성 목록에서 숨김, 연결 투두는 `categoryId` **유지** — 카테고리와 함께 목록에서 숨겨짐 | **옵션 유지** (앱에서만 숨김) |

- 숨김 상태는 SwiftData에 영구 저장 — 앱 재실행 후에도 **자동 활성화되지 않음** (동기화 시 이름·옵션 ID 예약)
- 숨김 해제는 사용자가 수동 전환 시에만
- **숨김 확인 alert**는 `CategoryView` 본문에 바인딩 (목록 스와이프·편집 시트 공통). 편집 시트에만 두면 스와이프 시 팝업이 뜨지 않고 `showArchiveAlert`가 true로 남아 재진입 시 오동작할 수 있음
...
**투두탭 플래너 선택 시트 (2026-09-12):**
- 왼쪽 위 플래너 아이콘+이름+▾ 탭 → 제목 「플래너」, 오른쪽 「완료」, medium detent
- 카드: 48pt 아이콘 / 이름(본문 bold) / 그 아래 상태 한 줄(14pt regular, secondary)
  - 노션 연동: 「노션에 연결됨」
  - 노션 미연동: **「기기에 저장」** (구 「로컬 저장」)
  - 구독 만료 잠금: 「읽기 전용」+ dim
- 맨 아래 「플래너 추가」(무료면 Pro 배지)
- 설정 → 플래너 상세 헤더도 미연동 라벨 「기기에 저장」(글씨 크기는 caption 유지)
...
- 모든 날짜 자유 이동 가능 (무료, 6-1 / 날짜 이동 규칙 참고)
...
enum CategoryStatus: String, Codable {
    case active    // 활성 (기본값)
    case archived  // 사용자 화면 라벨은 "숨김" (구 "보관" — 내부 case명은 유지, 노출 문구만 변경)
...
> **명칭·동작 변경 (2026-07-07):** 사용자향 기능명이 "보관"에서 "숨김"으로 변경됨. 기존엔 보관 시 연결 투두의 `categoryId`를 해제해 투두가 미분류로 남았으나, 현재는 **카테고리를 숨기면 연결된 투두도 함께 목록에서 숨겨짐** (`categoryId`는 유지 — 해제하지 않음). 이유: 미분류로 흩어지는 것보다, 카테고리 단위로 보이고 안 보이는 게 사용자가 이해하기 더 직관적임. 내부 `status` 값(`archived`)과 관련 파일명은 변경하지 않고, 노출 문구만 "숨김"으로 통일.
...
- 활성 목록에는 `status == .active`만 표시
- 연결 투두는 `categoryId` **유지** — 카테고리가 숨김 상태이므로 투두 목록에서도 함께 숨겨짐 (categoryId 해제 안 함)
- 노션 옵션은 유지 (앱에서만 숨김, 노션엔 그대로 남음)
- 노션 연동 플래너: 숨김 시 확인 팝업 — 「앱에서만 숨김, 노션에는 유지」
- 확인 alert는 `CategoryView` 루트에 바인딩 (`CategoryEditSheet` 전용 금지 — 스와이프 숨김 미동작·alert 상태 잔류 버그)

**숨김 해제:** 카테고리 설정에서 수동 전환 (정확한 UI 위치는 확인 필요 — 보관 목록에서 탭하는 방식이 그대로인지, 다른 토글로 바뀌었는지 니나 확인 요망)
...
우측: ⋯ 탭 → 보기 옵션 (네이티브 Menu)
```

### 보기 옵션 — 네이티브 Menu

⋯ 탭하면 시스템 Menu가 즉시 표시된다 (구 `.popover` + `ViewOptionsPopover` 대체).
...
```
투두탭 ⋯ → 「카테고리 설정」시트          설정 → 플래너 → 「카테고리 관리」push
┌─────────────────────────────────┐     ┌─────────────────────────────────┐
│ 편집          카테고리 관리    + │     │ ‹          카테고리 관리  편집 + │
└─────────────────────────────────┘     └─────────────────────────────────┘
  presentsAsSheet: true                   기본값 (플래그 없음). 화면·기능 동일
```

- 시트에는 `<` 뒤로가기가 없어서 편집을 왼쪽으로 옮김. 설정 push는 왼쪽이 이미 `<`라 **편집+추가를 오른쪽에 유지**.
- 이유: 시트에서 버튼 두 개가 trailing에만 몰려 보이던 레이아웃 보정. 설정 화면은 변경 없음.
...
**할일 메모 보기 동작:**
- 기본값: 꺼짐 (메모 숨김)
- 켜면: 메모가 있는 투두에만 제목 아래 미리보기 (최대 2줄, secondary)
- **글씨 (투두탭 `TodoRow`만, 2026-09-12):** 14pt regular. 변경 전 `.caption`(약 12pt) → 중간 `.footnote`(13pt) 폐기 → 확정 14pt. 제목 `.body`(약 17pt)보다 작고 읽기 쉽게.
- 인박스 시트·리포트 탭 메모/리뷰는 이 변경과 무관 (인박스 메모는 `.caption` 유지)

```
꺼짐 (기본):                        켜짐:
┌─────────────────────────────┐     ┌─────────────────────────────┐
│  ○  수학 문제 풀기           │     │  ○  수학 문제 풀기           │
│  ○  영어 단어 30개          │     │     p.132~145, 틀린 것 복습  │  ← 14pt 메모
│  ○  독서 30분               │     │  ○  영어 단어 30개          │
└─────────────────────────────┘     │  ○  독서 30분               │
                                    └─────────────────────────────┘
```
...
**할일 행 간격 (2026-09-12 확정):**
- 체크 원(`○`/채움)과 제목 사이 **8pt** (구 12pt). 「투두 추가」행의 +/빈 원과 텍스트도 동일
- 행 안 세로 padding **6pt** (구 8pt)
- `listRowInsets` top/bottom **3** (구 4 → 잠깐 2 → 확정 3), leading/trailing 24
- 칩이 있을 때 첫 할일 위 여백 **12pt → 16pt**. 칩이 없으면 리포트 카드~첫 할일 약 24pt 유지
- 이유: 맨 위는 답답하고 행 사이는 넓어 보였음. 가로 간격은 행간을 줄인 폭에 맞춤
...
│  ☐ 독서 30분                   │    우로 스와이프(풀스와이프): 고정
│                                 │    좌로 스와이프: 내일로/날짜 변경/삭제
│  + 투두 추가                    │    롱프레스: 컨텍스트 메뉴
└─────────────────────────────────┘
```
...
> - 자동 포커스: 인스턴스당 1회(즉시 시도 → 실패 시 ~2초 재시도). 인라인 추가는 `focusEpoch`+`.id`로 매번 fresh 인스턴스
> - 콜드 스타트: `KeyboardPrewarmer`(앱 시작 ~2.5초 뒤)로 키보드 세션 프리웜
> - 편집 시트 등: `autoFocus: false`로 자동 포커스 없음

> **[확정] 인라인 투두 입력 — 엔터 키 동작**
> - 이름 입력 후 엔터(`onSubmit`) → 현재 항목 저장 + 같은 위치에 새 입력 행 이어서 표시 (연속 추가)
> - 빈 입력 상태(글자 없음)에서 엔터 → 입력 종료, 포커스 해제
> - 세부 설정(카테고리·날짜·시간·알림)은 이 인라인 입력에서 불가 — 추가 후 편집 화면에서 설정
...
**고정 표시 (v1.09):** `isPinned == true`일 때 제목 옆 `pin.fill`(accent). 옛 「중요」 텍스트 태그(`ImportantTodoTag`)는 제거. **Notion 속성명 「중요」는 그대로 유지** (기존 사용자 DB 호환 — UI 명칭만 「고정」).
...
| 데이터 범위 | 선택 플래너의 로컬 SwiftData 투두 (`TodoItem`, 해당 날짜 기준) |
| 최대 조회 | 어제부터 최대 **365일** 거슬러 올라가며 연속 일수 집계 |
| 구현 | `StreakCriteria.isDaySatisfied` + `ReportService.calculateStreak` |

**「중요 할 일」의 정의:** 앱에서 `isPinned == true`(상단고정)인 투두. UI에는 「중요」 태그로 표시.

**기준별 — 그날이 연속 달성에 포함되는 조건** (`StreakCriteria.isDaySatisfied`):

| 기준 | 그날 달성 조건 | 할 일 0개인 날 | 중요 할 일 0개인 날 |
|---|---|---|---|
| **중요 할 일 모두 완료** | 중요 할 일(`isPinned`)이 **1개 이상**이면 → **전부 완료** | (할 일 유무 무관) **달성** | **달성** (완료할 중요 할 일이 없으면 그날 기준 충족) |
| **전체 할 일 완료** (기본) | 그날 투두 **1개 이상**이면 → **전부 완료** | **미달성** (연속 끊김) | 중요 유무와 무관 — 전체 기준만 적용 |
| **할 일 1개 이상 완료** | 그날 투두 중 **1개라도 완료** | **미달성** | **미달성** (완료된 항목이 없으면 끊김) |

**사용법 요약 (고객 안내용):**

- **중요 할 일 모두 완료:** 중요 표시한 할 일만 신경 쓰면 됨. 그날 중요 할 일이 없으면 자동으로 그날은 달성 처리. 일반 할 일만 있고 중요 할 일이 없어도, 중요 할 일 기준만 보면 달성.
- **전체 할 일 완료:** 그날 등록된 모든 할 일을 다 끝내야 함. 할 일이 하나도 없는 날은 달성으로 치지 않음.
- **할 일 1개 이상 완료:** 하루에 하나라도 체크하면 됨. 아무 것도 완료하지 않은 날은 끊김.

**표시:** 주간·월간 리포트 상단 「연속 달성 N일」. 설정 화면 푸터: 「리포트의 연속 달성은 어제까지 기준으로 계산됩니다.」
...
| 잠금화면 위젯 / Dynamic Island | v2 | 빠른 추가 레벨 3. 타임블로킹과 함께 검토 |
...
> 참고 (2026-07-07): 여기서 "보관"은 이후 사용자 노출 문구가 "숨김"으로 변경됨 (8-5절 참고). 코드 심볼(`showArchiveAlert`, `confirmArchive()` 등)은 당시 이름 그대로이며, 아래 내용은 변경 당시 기록 그대로 보존.
