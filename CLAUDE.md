...
- v1.17 제출 예정 (날짜 미정. 할 일 알림 문구·직접 입력 키패드·앱 사용 중 배너·필터 기억하기. 마케팅 버전·빌드 번호는 Xcode에서만. 아래 버그 수정은 main에 있음. **기분 칩은 노션 연동 전이라 1.17에 넣지 않음**)
...
- **알림 subtitle (`95c9b5b`):** 당시 리마인더에 예정 시각. **2026-09-23에 body 규칙으로 교체** (아래 「할 일 알림」).
...
백엔드 변경 없음 (iOS만). 커밋 `9da5043` · `e8f19e4` · `7d1fa1a`.

- **문구:** 제목은 할 일 이름. body는 `TodoNotificationManager.scheduleTimeText`. 울리는 날(`fireDate`)과 예정일의 날짜 차이로 「오늘 / 내일 / 모레 + 시각」. `Date.now`와 비교하지 않는다. 차이가 그 밖이면 `M월 d일 (요일) 시각`. 메모는 넣지 않는다.
- **취소:** `alarmOffset == nil`이면 기존 알림을 취소한다. `scheduledTime == nil`이면 기존 알림은 유지한다.
- **직접 입력:** 메뉴 「직접 입력」. 행의 「없음」/「완료」. 숫자 키패드 1~999, 단위 휠(분·시간·일·주·개월). 프리셋이 아닌 값은 메뉴에 표시된다. 다시 열면 저장된 값에서 시작한다.
- **앱 사용 중 배너:** `AppNotificationDelegate.willPresent`가 할 일 알림(identifier = `todo.id`)도 `.banner` · `.sound` · `.list`로 표시한다. 리포트 리마인더(`report-save-reminder`)와 탭 동작은 그대로다.
...
- 카테고리 색상 직접 선택 — 편집 시트 `Category.baseColors` 12색 + `ColorPicker` (1.08 팔레트 세트 9종 대체). `categoryPaletteSetId` 필드는 마이그레이션용으로만 유지
- What's New v1.0.9 (5항목) + notice(위젯 재추가 안내) + 팝업 할일 행 미리보기(`WhatsNewTodoRowPreview`) + 인라인 수정 1회 말풍선(`hasSeenInlineEditHint`)
...
- 카테고리 색 팔레트 세트 9종 (`CategoryPaletteSet`) — v1.08 추가 후 **2026-07-30 UI 제거**(`docs/archive/` 보관). v1.09에서 **직접 선택(`ColorPicker`)**으로 대체. `PlannerItem.categoryPaletteSetId`만 경량 마이그레이션용으로 유지
...
- 구독 만료 플래너 선택 안내: "읽기 전용" → "비활성화" (선택하지 않은 플래너는 재구독 전까지 사용 불가)
...
- ~~알림 메시지에 설정 시각 표시~~ — 완료. 2026-09-23부터 body에 「오늘/내일/모레 + 시각」(`fireDate` 기준). 아래 「할 일 알림」
...
- **기분 노션 연동** — 백엔드 GET 읽기·`POST /api/notion/daily-report/mood`는 배포됨(`6386eb4`). iOS는 아직 이 API를 호출하지 않음. 연동 전이라 **출시 불가**

### 최근 완료 작업
- 기분 3-3a 칩·편집 (2026-09-26, `8c1f091`): 하루 리뷰 카드 기분 칩·메뉴·편집 화면. 별 칸은 선택과 상관없이 같은 크기(⭐·`star.fill` 중 큰 쪽), 별은 20pt·간격 6, 줄 높이는 24pt(캡션이 더 크면 캡션). **노션 연동 전이라 출시 불가.** 백엔드 변경 없음
- 기분 3-2 데이터 계층 (2026-09-26, `cc58246`): `MoodOptionItem`, 기분 모드(nil/notion/appOnly/disabled), 자동 연결, 별점과 충돌 방지. 화면 연동은 3-3a. **출시 불가**
- 별점 「앱에만 저장」 (2026-09-26, `bca59ff`): 선택 유지(`ratingStoredInAppOnly`). 하루·기간 리포트 노션 저장·불러오기에서 별점 제외. 백엔드 별점 fallback은 이름 「별점」만 (`6386eb4`, 운영 배포됨)
- 하루 리뷰 오프라인 저장 (2026-09-26, `8aaf331`): `notionSyncPendingAt`이 있는 동안 노션의 옛 리뷰가 로컬을 덮어쓰지 않음. 연결 복구·앱 활성화 때 재전송
- 카테고리 오프라인 추가·삭제 차단 (2026-09-26, `471655e`): 노션 select/status 연동 플래너에서만. 오프라인 추가·삭제를 막아 다음 동기화 때 선택지가 되살아나지 않게 함
...
- 카테고리 팔레트 세트 9종 + 색 대비 (2026-07-23, v1.08) → 세트 UI 제거(2026-07-30) → **v1.09 직접 선택(`ColorPicker`)**으로 대체. 색 대비 `readableForeground`/`readableText`는 유지
...
- (대부분 해결, 빌드22) v1.10 위젯 App Group 이관 시 로컬 데이터 화면 소실 — 아래 「v1.10 위젯 데이터 손실 사건」참고. Application Support 자체가 없는 기기는 앱으로 복구 불가

### v1.10 위젯 데이터 손실 사건 (2026-08-31 ~ 09-07, 빌드 19~24)

**증상**: v1.10 업데이트 후 일부 사용자의 로컬 투두·카테고리·리포트가 화면에서 전부 사라짐.

**근본 원인**: v1.10에서 위젯이 App Group 공유 SwiftData 스토어를 직접 읽도록 변경(`AppGroupStore`)하면서, 위젯이 앱보다 먼저 실행되면 위젯이 자기 자신의(앱과 분리된) 샌드박스에서 "legacy 스토어 있나?" 체크를 하게 됨 → 항상 없다고 판단 → 빈 공유 스토어를 위젯이 먼저 만들어버림. 이후 메인 앱이 실행되면 "공유 스토어 이미 있네" 하고 그 빈 스토어를 그대로 채택 → 기존 로컬 데이터 접근 경로 상실.

여기에 더해 `PlannerService.setup()`의 "플래너 없으면 기본 플래너 자동 생성" 로직이 겹치면서, 그 순간 이름 없는 새 빈 플래너(예: "일상 기록")가 자동 생성되고 `selectedPlannerId`가 그쪽으로 덮어써짐 — 이후 legacy 데이터를 되살려도 선택된 플래너가 계속 가짜 쪽을 가리켜 화면엔 안 보이는 2차 문제 발생.

**수정 내용 (`Common/AppGroupStore.swift`, `TodoReport/Core/Persistence/PersistenceController.swift`)**:
1. 위젯 전용 read-only 설정 분리 — 공유 스토어 파일이 **이미 있을 때만** 사용, 없으면 아무것도 만들지 않고 nil 반환.
2. 메인 앱 시작 시 legacy(`Application Support/default.store`) → shared(App Group) **id 기준 병합**(파일 복사 방식 아님). 엔티티별로 개별 save + 실패 시 rollback (한 엔티티 실패가 다른 엔티티까지 안 막게).
3. "가짜 플래너 정리": `legacy∩shared` 플래너 id 교집합으로 "진짜" 플래너를 판별(이번 실행에서 병합됐는지 여부와 무관, 매 실행 멱등) → 그 외 플래너에 딸린 todo/category/report/syncQueue를 진짜 플래너로 재할당 → `selectedPlannerId`를 진짜 플래너로 복구.
4. heal 관련 로그를 OSLog뿐 아니라 `AppLogger`에도 남겨서, 사용자가 설정 → 고객지원 → 오류신고만 보내도 heal 진행 상황을 확인할 수 있게 함 (빌드 23~24).

**미해결/참고 — 실사용자 케이스 하나는 위 수정으로도 복구 안 됨**:
사용자 1명("정혜윤" 테스트 케이스) 기기에서 오류신고 로그 확인 결과, `Application Support` 디렉토리 자체가 기기에 존재하지 않음(파일이 있는데 못 찾는 게 아니라 폴더 자체가 없음) — legacy 스토어를 애초에 열 수 없는 상태였음. 이건 앱 코드가 지운 게 아니라(코드상 legacy를 삭제하는 경로 없음, 복사만 함), iOS 차원에서 그 컨테이너가 유실된 것으로 추정(원인 불명 — 오프로드 관련 가능성 등, 확인 불가). **이 경우는 앱 코드로 복구 불가, 사용자의 아이클라우드 백업 복원만 유일한 방법.**
→ 앞으로 비슷한 "복구가 전혀 안 된다"는 신고가 오면, 오류신고 로그에서 `Application Support 내용물 조회 실패` 여부부터 확인할 것 (빌드 24부터 이 진단 로그가 항상 찍힘).

**버전 히스토리**: 1.10(빌드19, 버그 유입) → 1.11(빌드20, 1차 병합 fix, 미완성 — 이미 병합된 경우 정리 로직 스킵되는 버그 있었음) → 1.12(빌드21, 정리 로직 추가했으나 "이번 실행에 병합했을 때만" 조건이라 여전히 불완전) → 1.12(빌드22, **실질 수정 완료** — 조건을 legacy∩shared 매 실행 판별로 변경) → 1.12(빌드23~24, 진단 로그만 추가, 기능 변경 없음).
...
**문체 (한국어 안내문):**
- 설명·상태 안내는 합쇼체 ("~됩니다", "~합니다")
- 사용자에게 행동을 요청할 때는 "~해 주세요"
- 해요체("~돼요", "~해요")는 신규 문구에 쓰지 않음
- 버튼·메뉴 라벨은 명사형 또는 짧은 동사형 유지 (문체 규칙 대상 아님)
- 영어는 기존대로 iOS 표준 톤, 간결하게
...
**색상 (로컬):** 기본값은 `Category.baseColors` 12색에서 미사용 우선 랜덤. 편집 시 12색 그리드 + `ColorPicker`로 직접 선택(v1.09). 대비는 `readableForeground` / `readableText(on:)` (`Common/Colors.swift`). (옛 팔레트 세트는 제거·`docs/archive/` 보관)
...
한 행에 버튼이 2개 이상이면 탭 한 번에 전부 실행된다. 각 버튼에 `.buttonStyle(.borderless)`나 `.plain`을 붙인다. 알림 직접 입력 행의 「없음」/「완료」에서 확인 (2026-09-23).
