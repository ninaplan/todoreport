...
- 달력 「이 달 불러오기」 — 노션 과거 데이터를 월 단위로 pull. 백엔드 GET에 `startDate`~`endDate` 기간 조회 **필드 추가만** (기존 단일 `date` equals 유지, 구버전 앱 호환). iOS `MonthCalendarView`에서 명시 버튼으로만 호출 (날짜 이동 시 자동 pull 금지 원칙 유지). 캘린더 탭 확장 시 동일 API·컴포넌트 재사용
...
- 달력 UX 미세조정 — 월 이동 시 선택 해제, 필터 Menu 틴트를 앱 포인트/세컨더리 톤에 맞춤, 날짜·목록 폰트, 말풍선 위치·삼각형 간격
